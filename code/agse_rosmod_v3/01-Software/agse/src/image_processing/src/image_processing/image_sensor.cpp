#include "image_processing/image_sensor.hpp"
...
void image_sensor::controlInputs_sub_OnOneData(const high_level_control::controlInputs::ConstPtr& received_data)
{
...
bool image_sensor::captureImageCallback(image_processing::captureImage::Request  &req,
  image_processing::captureImage::Response &res)
{
  // Business Logic for captureImage_server Server
...
void image_sensor::startUp()
{
  ...
  if (portGroupMap.find("captureImage_server") != portGroupMap.end())
    advertiseName += "_" + portGroupMap["captureImage_server"];
  ros::AdvertiseServiceOptions captureImage_server_server_options;
  captureImage_server_server_options = ros::AdvertiseServiceOptions::create<image_processing::captureImage>
      (advertiseName.c_str(),
